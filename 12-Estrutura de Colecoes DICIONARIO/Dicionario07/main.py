...
try:
    ...
    while True:
        ...
        prosseguir = input("Deseja remover alguma chave? (s/n): ").strip().lower()

        match prosseguir:
            case 's':
                chave_informada = input("Informe o nome da chave que deseja remover: ").strip().title()
                if chave_informada in chaves:
                    usuario.pop(chave_informada, None)
                    os.system('cls')
                    continue
                else:
                    os.system('cls')
